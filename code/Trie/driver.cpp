...
int main (int argc, char* argv[]){
    ...
    for (int i = 0; i < 400; i++ ){
        ...
        for (int j = 0; j < 100; j++){
            T.add(testData[index]);  // add tracking IDs 100 at a time
            index++;
        }
        ...
        for (int j = 0; j < 100; j++){
            ...
            bool flag = T.search(testData[ind]);
            if (!flag){  // check to make sure search was succesful
                cerr << "One or more searches failed!!!" << endl;
                return 1;
            }
        }
    ...
    T.deleteList();  // delete the Trie when finished with it so we dont leak memory


    // read data into a csv file
    for(int i = 0; i < 4; ++i)
        DATAFILE.pop_back(); // drop ".csv" from file being read
    ofstream outFile("insert_search_performance_Trie_" + DATAFILE + ".csv");
